...
async function main() {
  console.log('📂 Reading question paper JSON file for 24 Jan Shift 1...');
  const jsonPath = path.join(__dirname, '..', 'src', 'papers', 'JEE Main 2025 (24 Jan Shift 1).json');
  ...
  // 2. Find or create the Shift: "24 Jan - Shift 1"
  const shiftName = '24 Jan - Shift 1';
  ...
  if (shift) {
    ...
    shift = await prisma.shift.create({
      data: {
        examId: exam.id,
        name: shiftName,
        date: new Date('2025-01-24T09:00:00Z') // Set exam date to 24 Jan 2025
      }
    });
  }
